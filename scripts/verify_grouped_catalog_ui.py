...
def main():
    ...
    checks = {
        ...
        "catalog enrichment comment exists": "Product Catalog Enrichment Layer" in html and "Phase 1: prepares product display metadata" in html,
        "catalog products are derived from mbm products": "const catalogProducts = mbmProducts.map(enrichProductForCatalog)" in html,
        "catalog helper functions exist": all(name in html for name in [
            "catalogImagePlaceholders",
            "productTypeRules",
            "productCategoryMeta",
            "slugifyProductIdentifier",
            "hasValidLumen",
            "getProductType",
            ...
            "enrichProductForCatalog",
        ]),
        "catalog enriched fields exist": all(field in html for field in [
            "display_name",
            "short_name",
            "catalog_category",
            "product_type",
        ...
        "catalog image paths are safe placeholders": all(text in html for text in [
            "getExpectedProductImagePath",
            "getExpectedApplicationImagePath",
            "./images/products/placeholder-product.webp",
            "./images/applications/placeholder-application.webp",
        ]),
        "phase 2a catalog card components exist": all(text in html for text in [
            "function ProductCategoryCards",
            "function ProductCategoryCard",
            "function ProductImage",
            "function ProductSpecsChips",
            "function ProductBadges",
            "function ProductUses",
        ]),
        "phase 2a view mode toggle exists": "const [viewMode, setViewMode] = useState('cards')" in html and "aria-pressed={viewMode === 'cards'}" in html and "aria-pressed={viewMode === 'table'}" in html,
        "phase 2a cards use category cards": "catalogProducts.filter" in html and "catalogCategoryCards.filter" in html and "ProductCategoryCards cards={filteredCategoryCards}" in html,
        "phase 2a cards are default view": "useState('cards')" in html and 'data-role="catalog-cards-view"' in html,
        "phase 2a keeps table view": "viewMode === 'table'" in html and 'data-role="desktop-product-table"' in html and 'data-role="mobile-product-groups"' in html,
        "phase 2a image fallback exists": 'data-role="product-image-placeholder"' in html and "الصورة قيد التحديث" in html and "onError" in html,
        "phase 2a has no modal or calculator binding": all(text not in html for text in [
            "ProductModal",
            "DetailModal",
            "catalog-modal",
            "استخدم هذا المنتج",
        ]),
        "phase 2a fix family grouping exists": all(text in html for text in [
            "Product Family Grouping Layer",
            "groupCatalogProductsIntoFamilies",
            "const catalogFamilies = groupCatalogProductsIntoFamilies(catalogProducts)",
            "const catalogFamilies = groupCatalogProductsIntoFamilies(catalogProducts)",
        ]),
        "phase 2a fix 2 category card grouping exists": all(text in html for text in [
            "Category-Level Catalog Cards Layer",
            "groupCatalogProductsIntoCategoryCards",
            "const catalogCategoryCards = groupCatalogProductsIntoCategoryCards(catalogProducts)",
            "function ProductCategoryCard",
        ]),
        "phase 2a fix 2 cards use category cards": "function ProductCategoryCards({ cards, onOpenDetails })" in html and "ProductCategoryCard key={card.card_id}" in html and "ProductCategoryCards cards={filteredCategoryCards}" in html,
        "phase 2a fix category filter exists": "selectedCategoryFilter" in html and "الفئة" in html and "كل الفئات" in html and "قواعد إضاءة كوب" in html,
        "phase 2a fix export is table only": "viewMode === 'table' && (" in html and "onClick={exportFilteredProductsToExcel}" in html,
        "phase 2a fix product tab is catalog": "كتالوج المنتجات" in html and "كتالوج منتجات MBM LIGHTING" in html,
        "phase 2a fix family badges hide calculator badge": "hiddenFamilyCardBadges" in html and "مناسب للحاسبة" in html and "لومن متوفر" in html,
        "phase 2a fix 2 does not use model families for cards": "ProductCatalogCards families={filteredFamilies}" not in html and "ProductFamilyCard key={family.family_id}" not in html,
        "phase 2b modal components exist": all(text in html for text in [
            "function CategoryDetailsModal",
            "function CategoryDetailsHeader",
            "function CategoryDetailsImages",
            "function CategorySummaryGrid",
            "function CategoryVariantTable",
            "function VariantStatusBadges",
            "function DetailChipList",
        ]),
        "phase 2b fix responsive variant components exist": all(text in html for text in [
            "function VariantMobileCards",
            "function VariantMobileCard",
            "function VariantThumbnail",
            "function CommonFeaturesBlock",
            "function VariantTechnicalDetails",
            "function ExpandableVariantDetails",
            "const shouldShowVariantImages",
            "const getVariantImagePath",
            "const collectCommonFeatures",
            "const shouldShowUgrColumn",
            "const getCompactVariantName",
        ]),
        "phase 2b modal state exists": "selectedCategoryCard" in html and "setSelectedCategoryCard" in html and "Boolean(selectedCategoryCard)" in html,
        "phase 2b card opens modal": "onOpenDetails(card)" in html and "عرض التفاصيل" in html and "ProductCategoryCards cards={filteredCategoryCards} onOpenDetails={setSelectedCategoryCard}" in html,
        "phase 2b modal uses category products": "card.products" in html and "sortCategoryVariants" in html and "CategoryVariantTable products={card.products}" in html and "VariantMobileCards products={card.products}" in html,
        "phase 2b modal accessibility exists": 'role="dialog"' in html and 'aria-modal="true"' in html and "aria-labelledby" in html,
        "phase 2b escape close exists": "event.key === 'Escape'" in html and "window.addEventListener('keydown'" in html and "window.removeEventListener('keydown'" in html,
        "phase 2b variant fields exist": all(text in html for text in ["رقم الصنف", "اسم الصنف", "الموديل", "الواط", "الكيلفن", "اللومن", "CRI", "الزاوية", "UGR"]),
        "phase 2b fix responsive views exist": "hidden rounded-xl border border-slate-700 bg-slate-950 shadow-xl md:block" in html and "block space-y-3 md:hidden" in html,
        "phase 2b fix desktop table is compact": "جدول مختصر لكل SKU" in html and '<th className="p-3 font-bold">اسم الصنف</th>' not in html and '<th className="p-3 font-bold">المواصفات الفنية</th>' not in html and '<th className="p-3 font-bold">المميزات</th>' not in html,
        "phase 2b fix common features block exists": "مميزات مشتركة في هذه الفئة" in html and "getUniqueCommonFeatures(card.products)" in html,
        "phase 2b fix expandable details exist": "aria-expanded={isExpanded}" in html and "تفاصيل إضافية" in html and "VariantTechnicalDetails product={product}" in html,
        "phase 2b fix variant thumbnails are conditional": "shouldShowVariantImages(card)" in html and "showImages={showVariantImages}" in html and "getVariantImagePath(product)" in html,
        "phase 2b has no calculator action": "استخدم هذا المنتج في الحاسبة" not in html and "استخدم في الحاسبة" not in html,
